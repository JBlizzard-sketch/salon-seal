...
export default function Bookings() {
  ...
  const handleSendReminder = (id: number, phone: string) => {
    ...
    sendReminder.mutate(
      { id, data: { type: "manual" } },
      {
        onSuccess: (data) => {
          ...
          setReminderSent((p) => ({ ...p, [id]: data.waMessageId }));
        ...
        {isLoading ? (
          ...
          <div className="divide-y">
            {bookings.map((booking) => {
              ...
              const alreadySent = sentBookingIds.has(booking.id) || !!reminderSent[booking.id];
                  ...
                  <div className="flex gap-2 shrink-0 flex-wrap justify-end">
                    {canRemind && !alreadySent && (
                      <Button
                        ...
                        disabled={isSending}
                        onClick={() => handleSendReminder(booking.id, booking.clientPhone)}
                      >
                        {isSending ? (
                          <>
                    ...
                    {booking.status === "confirmed" && (
                      <Button size="sm" variant="outline" onClick={() => handleStatusUpdate(booking.id, "arrived")}>
                        Mark Arrived
                      </Button>
                    )}
